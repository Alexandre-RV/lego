...
const BEST_DEALES = [
    {
        'name': 'Deal Dealabs',
        'url': 'https://www.dealabs.com/bons-plans/jeu-de-construction-lego-star-wars-75375-un-nouvel-espoir-millenium-falcon-via-44eur-sur-carte-fidelite-2885128'
    },
    {
        'name': 'Deal Avenue de la brique',
        'url': 'https://www.avenuedelabrique.com/lego-movie/70828-le-bus-discotheque/p5212'
    }
];
...
const communityNames = [...new Set(deals.map(deal => deal.community))]; // Unique community names
...
console.log("Sorted by date (recent to old):", sortedByDate);
...
Object.entries(communities).forEach(([community, deals]) => {
    console.log(`Community: ${community}, Number of deals: ${deals.length}`);
});
...
Object.entries(communities).forEach(([community, deals]) => {
    const sortedByPrice = [...deals].sort((a, b) => b.price - a.price); // Descending
    console.log(`Community: ${community}, Deals sorted by price:`, sortedByPrice);
});
...
Object.entries(communities).forEach(([community, deals]) => {
    const sortedByDate = [...deals].sort((a, b) => new Date(a.date) - new Date(b.date)); // Oldest to recent
    console.log(`Community: ${community}, Deals sorted by date:`, sortedByDate);
});
...
const p5Price = computePercentile(prices, 5).toFixed(2);

const p25Price = computePercentile(prices, 25).toFixed(2);
...
// Define the threshold date (3 weeks ago)
const threeWeeksAgo = new Date();
threeWeeksAgo.setDate(threeWeeksAgo.getDate() - 21);

// Check for very old items
const hasVeryOldItems = VINTED.some(item => new Date(item.published) < threeWeeksAgo);
console.log("Very old items exist:", hasVeryOldItems);

// 🎯 TODO 13: Find a specific item
// Find the item with the specific uuid
const specificItemUUID = "f2c5377c-84f9-571d-8712-98902dcbb913";
const specificItem = VINTED.find(item => item.uuid === specificItemUUID);
console.log("Specific item:", specificItem);

// 🎯 TODO 14: Delete a specific item
// Remove the specific item with the uuid
const updatedVintedList = VINTED.filter(item => item.uuid !== specificItemUUID);
console.log("Updated list after deletion:", updatedVintedList);

// 🎯 TODO 5: Save a favorite item
// Make a copy of `sealedCamera` and modify its price

// Step 1: Copy sealedCamera to camera and set a new property
let sealedCamera = {
    link: "https://www.vinted.fr/items/5563396347-lego-43230-omaggio-a-walter-disney-misb",
    price: "131.95",
    title: "LEGO 43230 omaggio a Walter Disney misb",
...
let camera = { ...sealedCamera }; // Create a shallow copy
camera.favorite = true;

// 1. Log `sealedCamera` and `camera`
console.log("sealedCamera:", sealedCamera);
console.log("camera:", camera);
// Observation: Adding the `favorite` property to `camera` does not affect `sealedCamera` as we created a copy.

// Step 2: Update sealedCamera
sealedCamera = {
    link: "https://www.vinted.fr/items/5563396347-lego-43230-omaggio-a-walter-disney-misb",
    price: "131.95",
    title: "LEGO 43230 omaggio a Walter Disney misb",
...
// Step 3: Update `camera` without changing `sealedCamera`
...
// 1. Compute the potential highest profitability
